  ...
  return (
    <div className="section-shell section-space">
      ...
      <FadeSection className="max-w-4xl">
        <h1 className="text-4xl font-extrabold text-foreground sm:text-5xl">{t("about.title")}</h1>
      </FadeSection>
      ...
      <FadeSection className="mt-14" delay={0.15}>
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {team.map((member) => (
            <Card key={member.name} className="rounded-lg">
              <CardContent className="p-6">
                <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-muted text-lg font-bold text-primary">{member.name.split(" ").map((part) => part[0]).join("")}</div>
